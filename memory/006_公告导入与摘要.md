...
| 分组 | 字段 | 当前语义 |
| --- | --- | --- |
| 主键与归属 | `id`、`company_id` | `id` 是本地数据库自增主键；所有读取、删除、摘要和下游引用都按公司范围校验 |
| 公告元数据 | `title`、`published_at`、`category` | 标题、UTC 发布时间和分类；深度摘要可修正 `category` |
| 来源 | `source`、`source_url`、`raw_url` | 来源名、可浏览详情页、PDF 原文地址 |
| 正文 | `content`、`raw_content` | `content` 是兼容旧数据的正文回退字段；当前新抓取正文写入 `raw_content` |
...
- 没有独立的 `source_announcement_id`、`art_code` 字段。
- 没有 `content_source`、`content_fetched_at` 字段。
...
- 东方财富 `art_code`：例如 `AN202607171827064564`。当前不单独入库，而是嵌在 `source_url` 和 `raw_url` 中，正文读取时用正则 `AN\d+` 提取。

去重不是按独立“来源公告 ID”字段完成，而是：

1. 优先匹配同一公司下完全相同的 `source_url`；数据库也有 `(company_id, source_url)` 唯一约束。
2. 找不到时回退匹配 `(company_id, title, published_at)`。
...
顶部提供“一键快速摘要”“一键深度摘要”“搜索公告”。“搜索公告”实际是从东方财富同步，不是本地关键字搜索。
...
- 正文来源类型和抓取时间未持久化，无法仅凭数据库区分 notice API、PDF 或 HTML。
- 单条“深度摘要”正文失败时会静默降级为快速摘要，前端操作成功文案仍写“已生成单条深度摘要”，需要以后改成按返回模型名提示。
- 已存在公告除页面壳修复外会直接跳过，东方财富后续修正标题、分类或 URL 时不会自动覆盖本地值。
- 同步清理作用于公司下全部公告来源，未来增加手工公告来源前应拆分保留策略。
