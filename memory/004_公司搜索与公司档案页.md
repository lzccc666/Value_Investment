...
- 标准种子共 75 家：A 股 48 家、港股 5 家、美股 22 家，共 78 个 Listing；包含用户点名的 PDD、Circle、泡泡玛特、OXY、美国运通、美国银行、雪佛龙、安达保险、穆迪、卡夫亨氏、达美航空、天狼星 XM 和 Credo，并让伯克希尔 BRK.A/BRK.B、Alphabet GOOG/GOOGL 分别共用同一 Company。
- 当前真实项目库已同步到 schema v9，共 99 家公司/102 个 Listing；其中标准种子为 75/78，额外 24 家为用户通过 SEC 目录导入的公司，启动初始化不会删除或合并这些用户数据。
...
015 另提供面向持仓录入的 SEC 美股目录接入：`GET /api/investment-tools/portfolio-listings/us-catalog/search` 搜索 SEC 官方 ticker/CIK/exchange 目录并通过证券目录过滤普通股/ADS，`POST /api/investment-tools/portfolio-listings/us-catalog/import` 再次核对 submissions 的 ticker、交易所和证券类型后幂等创建本地 Company/Listing。仅接受 Nasdaq/NYSE，明确排除 ETF、优先股、权证、OTC/CBOE；`entityType=other` 只有存在 `20-F/6-K` 境外发行人记录时才可导入。自动导入的证券单位比例保持未核验 `NULL`，不会绕过 011 的 share-ratio 门禁。
...
2026-08-22：公司公共档案不再因工作区 section 不同而裁剪。多视角分析、备忘录准备和无锚定估值界面与其他 section 一样，完整显示基础档案、行情更新时间、价格、市值、估值倍数、行情/FX capability 和行情刷新入口。该调整只属于前端研究上下文展示，不改变 008-010 的请求数据、输入快照、模型提示词或确定性计算。
...
- SEC 真实档案/财务/披露刷新要求配置含联系邮箱的 `SEC_USER_AGENT`；未配置时 capability 明确为 blocked。
- 港股结构化数据依赖固定版本 AKShare，HKEXnews 是披露审计来源；上游字段或网页结构变化会明确降级或报错，不静默猜值。
- 首轮只允许 A 股、港交所和 NASDAQ/NYSE/AMEX 的 common stock/ADS 进入完整链路；其他证券类型不因能搜索到就声明支持。
...
## 014 多市场基础层当前实现（2026-08-21）

- `Company` 已固定为发行人级研究根，增加 `canonical_key/legal_name/aliases/domicile_country/reporting_currency/fiscal_year_end/external_ids`；旧 ticker、exchange 和行情字段继续作为主 Listing 兼容镜像。
- 已新增 Listing 列表/创建、主 Listing、能力状态、Listing 档案刷新、不可变行情刷新与最新快照 API。公司搜索同时匹配发行人名称、别名及各 Listing 的 ticker/symbol/exchange/market。
- 现有 A 股档案、行情、财务和公告已迁入 Provider 注册表；未传 `listing_id` 的旧 API 继续使用主 Listing。
- 美股 SEC 档案 Provider 已完成 Apple 固定样本验证。港股已注册固定版本 AKShare 证券/公司档案 Provider；腾讯固定样本和显式真实 smoke 均验证 legal name、别名、注册地、财务币种、财政年结日、ISIN 与上市日期。
- 前端公司工作台已提供 Listing 选择、Provider capability 状态、动态交易币种、Listing 级档案/行情刷新和不可用原因；双 Listing 测试验证切换到 `BABA.US` 后财务、公告和 011 请求均携带所选 Listing ID，次级 Listing 不回退串用主 Listing 行情。
- 阿里巴巴种子使用同一个 `Company` 承载 `09988.HK` 普通股和 `BABA.US` NYSE ADS；BABA 明确冻结 `1 ADS = 8 ordinary shares`，切换主 Listing 只更新 Company 兼容镜像，不改写既有估值、行情或价格决策。用户新建 ADS 时比例不再默认猜测，缺失比例由 011 明确阻断。
- 2026-08-21 浏览器验收确认：贵州茅台原 A 股工作台、腾讯 HKD 行情刷新、阿里港美双 Listing 选择和 Provider 阻断原因均可见；390px 下 Listing 面板改为单列，主 Listing 选项使用短标记“主”，能力标签和操作按钮无重叠或文字裁切。
- 2026-08-22 真实档案复验：本机已配置合规 SEC User-Agent，Apple 与 BABA.US 的档案/财务/披露 capability 均为 available。SEC submissions 缺少 `fiscalYearEnd` 时，档案 Provider 从最新年度 `10-K/20-F` 标准事实的显式期末日推导月日；BABA.US 浏览器显示 `Alibaba Group Holding Ltd / CNY / 03-31`。启动种子只补发行人档案空值并合并缺失外部 ID，重启后不再把该 Provider 档案覆盖回旧种子值。
- 2026-08-22 分红能力改为发行人数据感知：美股 Provider 的基础能力仍是 `partial`，因为 SEC 标准现金分红事实并非每个发行人、每个期间都存在；若所选公司已经落库任一数值型 `dividend_payment`，能力 API 返回 `available`。Apple 最新累计期间已读取 117.78 亿 USD 现金分红，因此工作台显示“分红：可用”；没有事实的公司仍显示“部分可用”及原因，不被 Apple 的结果误覆盖。
- 前端只翻译展示标签，不更改 API/数据库字段名。财务类型、期间类型、来源名、缺口字段、SEC 表单、公告分类和语言优先显示中文；原始英文标识继续用于后端合同和溯源。

## 015 复用边界（2026-08-22）

015 持仓组合通过现有公司搜索和 Listing API 选择具体证券，并复用本模块的 Listing 行情刷新服务与不可变 `MarketSnapshot`。它不回写 Company 兼容行情字段以外的新研究事实，也不改变公司工作台、004 刷新合同或 011 的快照冻结逻辑。组合详情只读取每个 Listing 的最新可用快照；批量刷新按 Listing 去重且隔离单项失败。东方财富返回的无时区行情时间在 015 响应边界按 `Asia/Shanghai` 解释，数据库快照原值和其他模块的序列化合同不变。

2026-08-22：015 不再复用 004 的“公司列表 -> 公司 Listing 列表”两段选择 UI，而是在独立 `GET /api/investment-tools/portfolio-listings/search` 中直接返回可录入 Listing。该端点仍读取同一 `Company/SecurityListing` 主数据，不改变 004 公司搜索响应、分页、相关性或公司工作台。搜索归一化支持点号代码和显式市场词；Provider 维护的 `Company.aliases` 不由种子补种覆盖。

标准公司种子现为 75 家/78 个 Listing：在 PDD、Circle、泡泡玛特和 OXY 之外增加美国运通、美国银行、雪佛龙、安达保险、穆迪、卡夫亨氏、达美航空、天狼星 XM 和 Credo；`BRK.A.US` 与 `BRK.B.US`、`GOOG.US` 与 `GOOGL.US` 分别共享同一 Company，`BABA.US` 与 `09988.HK` 继续共享阿里 Company。015 直接搜索固定回归覆盖中文名、英文名、简称、ticker、点号归一化和显式市场词。
...
| `backend/app/db/init_db.py` | 75 家标准公司/78 个 Listing、幂等写入和跨市场身份升级 |
...
### 2026-08-22 015 A/H 外部目录边界

- 015 的持仓录入新增独立 A/H 证券目录搜索与导入，不改变 004 公司池分页、公司档案页或研究工作区 API。目录导入后仍落入统一 `Company/SecurityListing` 主数据，后续本地搜索和行情刷新复用现有服务。
- 外部候选按 `quote_id` 二次核验，只接收沪深北和港股 HKD 普通股；同名或完全一致别名会复用现有发行人，其他情况保守创建新 Company，避免凭相似名称错误合并。新候选的中文名、证券代码和拼音缩写写入别名，支持本地简称搜索。
- TME 一类 SEC 境外发行人会在 `20-F/6-K` 与外部 ADS 类型双重核验后导入；外部中文名同时写入 Company 别名，后续导入同名港股时可复用同一发行人。ETF 和无法确认证券类型的条目不会进入 Company/Listing 主数据。
- 最终运行态已实际导入 `TME.US / NYSE / ads` 并把“腾讯音乐”写入别名；015 下拉用“腾讯音乐”同时返回本地 TME ADS 和外部 `01698.HK` 普通股候选。`BRK-A/BRK-B` 的 SEC 表示统一归一为 `BRK.A/BRK.B`，不会与本地股类重复。
