...
- 口径审计：基准期间类型、来源期间、标准化方法、标准化置信度、调整和警告。

### 非年报标准化

最新期是季报或中报时，不直接把累计单期 FCF 当成年度 DCF 基数。系统优先尝试：

1. 用最新累计期 + 上一完整年度 - 去年同期构造 TTM。
2. 无法构造 TTM 时，使用最近三个完整年度 FCF，按 `50%/30%/20%` 加权。
3. 只有两年或一年时使用可用年度并降低置信度。
4. 完整年度和可构造 TTM 都缺失时，报告估值输入缺口。

标准化 FCF 还会结合匹配口径净利润检查现金转化，并受 `fcf_profit_cap = 1.30` 上限约束。010 的增长来源优先级为：
...
- `buyback_amount` 仍缺真实期间回购现金流，库存股只是代理。
- `share_dilution_rate` 已进入证据包结构，但当前采集层没有按历史总股本自动计算。
- 分红来自现金流量表累计现金支出，不等同于公告口径 TTM 股息率；行情页股息率属于 004 的独立口径。
