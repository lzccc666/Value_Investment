...
## 014 市场化来源路由（2026-08-21）

- 007 的普通网页 fallback 根据 Company 的主 Listing/指定市场构造来源：A 股使用 SSE/CNInfo 与中国统计来源；港股使用 HKEX、SFC、香港政府及政府统计处；美股使用 SEC、BEA、BLS 与 Census。
- 港股和美股的确定性 seed source 与搜索 query 均不会落入 SSE/CNInfo。发行人快照增加非价格的注册地、报告币种、会计准则、披露语言/表单与 Listing 上下文，仍不读取行情、FX、市场估值或交易动作。
- 聚焦回归 `36 passed`，包含 A/H/美三类来源路由的直接断言。
