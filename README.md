...
本项目是一个本地优先的价值投资研究工作台。系统把公司主数据、财务底稿、公告、外部证据、分析师视角、投资备忘录、无价格锚定估值和价格决策串成一条可追溯的研究链路，并提供与研究链路隔离的投资小工具。
...
系统现已支持 A 股、港交所普通股以及 NASDAQ/NYSE/AMEX 普通股的统一研究合同。`Company` 是发行人级研究根，`SecurityListing` 承载同一发行人的多地上市证券；Apple `AAPL.US`、腾讯 `00700.HK` 和阿里巴巴 `09988.HK`/`BABA.US` 均有固定离线样本覆盖 004-011。行情与 FX 每次刷新创建不可变快照，011 冻结 Listing、币种、证券单位比率、行情、汇率及换算审计。

独立运维模块 `012 数据管理` 不计入研究主链路的 8 个模块。它通过侧边栏的“数据管理”入口提供数据库概览、两阶段数据清理、版本保留、SQLite 一致性备份、恢复、初始化和压缩维护；概览页仍显示 `004-011 / 8 / 8`。

独立模块 `015 投资小工具` 同样不计入研究主链路。它提供按具体 Listing 和证券数量记录的历史持仓快照、基于最新行情与 FX 的多币种组合估值，以及 A 股 QVIX、港股 VHSI、美股 VIX 的日频市场温度；不接入账户、交易或收益归因。
...
- 当前是单机研究工具；015 只维护人工录入的持仓数量和派生估值，不包含券商账户同步、成本收益归因或交易执行。
