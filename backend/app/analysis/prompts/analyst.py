...
PROMPT_VERSION = "analyst_view_v6"
...
- 不要使用联网结果替换快照中已有的确定性财务数据；事实冲突时优先保留冲突和不确定性，
  不得把搜索摘要扩写成网页未支持的结论。
- 008 必须彻底 price-blind。只分析业务经营、会计、现金流、资产负债、资本配置、
  管理治理、竞争与行业条件；不得读取、推断或输出下游市场对照、交易决策或仓位决策内容。
- JSON 的字段名与所有文本值都不得命名、复述或解释被排除的下游概念，也不要把它们
  写成“未评估某项”“无法判断某项”或数据缺口；输出前必须静默删除整句越界内容。
...
- financial_evidence_pack.model_display 是后端确定性生成的模型展示口径。
  写金额时必须使用 latest_amounts_100m 或 amount_series_100m 中的 display；
  display 已包含发行人真实 reporting currency，禁止擅自改写成 CNY；
...
- 可以基于利润表结构化字段输出财务质量判断、风险、反方证据和估值假设建议；
  不要计算每股内在价值，不要给出任何交易或仓位决策。
- “估值假设建议”只写后续研究应补充的经营和财务口径，例如利润可持续性、
  现金流增长、资本开支、会计调整和业务情景变量；不得讨论下游市场对照变量。
...
def build_analyst_prompt(
    *,
    ...
    retry_instruction = (
        "上一次输出越过了下游决策边界。请从头重新生成；所有字段名和文本只能描述业务经营、"
        "会计、现金流、资产负债、资本配置、管理治理、竞争或行业条件，不得命名任何被排除的"
        "下游概念。\n\n"
    ...
    return retry_instruction + "\n".join(
        [
            ...
            (
                "- 只输出业务经营、会计、现金流、资产负债、资本配置、管理治理、竞争和行业判断；"
                "不得输出下游市场对照、交易决策或仓位决策内容。"
            ),
            (
                "- 最终 JSON 不得命名或复述被排除的下游概念及其技术字段名，也不得把它们写成"
                "未评估项、无法判断项或数据缺口。输出前静默自检并删除整句越界内容。"
            ),
            (
            ...
            (
                ...
                "金额读取 latest_amounts_100m 或 amount_series_100m 的 display，"
                "display 已包含真实 reporting currency，不得假设为 CNY；"
            ...
            (
                "- 不要计算每股内在价值，不要输出任何交易或仓位决策；"
                "008 只输出质量判断、风险、反方证据和经营财务假设建议。"
            ),
            (
